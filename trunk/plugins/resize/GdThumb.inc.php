...
class GdThumb extends ThumbBase
{
	 ...
	 * @return GdThumb
	 */
	public function resize ($maxWidth = 0, $maxHeight = 0, $bgimage=false)
	{
		...
		$bg = imagecolorallocate($this->workingImage,$this->options['bgcolor'][0], $this->options['bgcolor'][1], $this->options['bgcolor'][2]);
		imagefill($this->workingImage, 1, 1, $bg);	
		if($bgimage==true) {
		if($this->newDimensions['newWidth'] < $maxWidth)
		$gotoX=($maxWidth-$this->newDimensions['newWidth'])/2;
		else
		$gotoX=0;
		if($this->newDimensions['newHeight'] < $maxHeight)
		$gotoY=($maxHeight-$this->newDimensions['newHeight'])/2;
		else
		$gotoY=0;		
		} else {
		$gotoX=0;
