<html>
    ...
    $('#button').click(function() {
        ...
        $.post('<?=homeinfo?>/ajax/savedescriptionpromotion','target=title&sid=<?=$this->data['sid']?>&value='+$('#textarea').val(), function(reposnse)         
      {
         eval("var obj1="+reposnse); 
          if(obj1.resposne){
              $('#title',parent.document.body).html($('#textarea').val());
              $('#title',parent.document.body).removeClass("nocontenttitle").addClass("contenttitle");
