<html>
    ...
    $('#textarea').html($('#description',parent.document.body).html());
    $('#button').click(function() {
        
        
        
        $.post('<?=homeinfo?>/ajax/savedescriptionpromotion','target=description&sid=<?=$this->data['sid']?>&value='+$('#textarea').val(), function(reposnse)         
      {
         eval("var obj1="+reposnse); 
          if(obj1.resposne){
              $('#description',parent.document.body).html($('#textarea').val());
              $('#description',parent.document.body).removeClass("nocontentdescription").addClass("contentdescription");
