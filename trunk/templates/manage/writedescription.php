<html>
    ...
    $('#textarea').html($('#detail',parent.document.body).html());
    $('#button').click(function() {
        
        
        
        $.post('<?=homeinfo?>/ajax/savetext2','target=description&meid=<?=$this->data['meid']?>&mid=<?=$this->data['userid']?>&value='+$('#textarea').val(), function(reposnse)         
      {
         eval("var obj1="+reposnse); 
          if(obj1.resposne){
              $('#detail',parent.document.body).html($('#textarea').val());
